...
const { BufferedSerial } = require("./buffered-serial");
...
async function get(serial, srcPath, destPath, fileSize) {
  ...
  // run code
  const code = `(${filePutCode})('${destPath}', ${bytesRemained}, ${packetSize});`;
  ...
  // send data
  process.stdout.write(`Copying ${filename} (host) to ${destPath} (device) `);
